...
export function isNumeric(value) {
  // Number.isFinite는 NaN, Infinity, undefined, null 등을 모두 false로 처리합니다.
...
export function isPositive(value) {
  const num = safeNumber(value, -1); // 기본값을 -1로 하여 유효하지 않은 값은 false가 되도록 유도
  return num > 0;
}
...
export function isInteger(value) {
  return Number.isInteger(safeNumber(value));
}
 ...
 * @param {string} currency - 통화 코드 ('KRW', 'USD') (style이 'currency'일 때 필수)
...
export function formatNumber(
  ...
  try {
    ...
    if (style === "currency") {
      ...
      options.currencyDisplay = "symbol"; //symbol, code, name
    }

    // toLocaleString을 사용하여 숫자 포맷팅
    return num.toLocaleString(locale, options);
  } catch (error) {
    // 로케일이나 옵션이 잘못된 경우를 대비한 안전 장치
...
export function formatCurrency(value) {
  const currency = formatNumber(value, "ko-KR", "currency", "KRW");
  const result = currency.replace("₩", "");
  return result;
}
